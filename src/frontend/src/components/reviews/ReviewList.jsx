...
const ReviewList = ({ gameId, releaseDate, game }) => {
  const { user } = useAuth();
  const { reviews, userReviews, isLoading, error, fetchGameReviews, fetchUserReviewForGame } = useReviewStore();
  ...
  const userReview = userReviews[gameId];
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        ...
        if (user) {
          await fetchUserReviewForGame(gameId);
        }
      } catch (err) {
        if (user) {
    ...
    fetchReviews();
  }, [gameId, fetchGameReviews, fetchUserReviewForGame, user]);
          ...
          <div className="flex items-center gap-2">
            ...
            {user && isReleased && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-primary text-black rounded-lg font-semibold text-sm hover:bg-primary/90 transition-colors cursor-pointer"
              >
                <PenSquare size={16} />
                  ...
                  <div className="absolute right-0 top-full mt-1 w-36 py-1 bg-surface-dark rounded-lg shadow-lg border border-gray-800/50 z-10">
                    {SORT_OPTIONS.map((option) => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-1.5 text-sm transition-colors duration-200 ${sortBy === option.value
            ...
            {sortedReviews.length > 0 ? (
              sortedReviews.map(review => (
                <ReviewItem
                  ...
                  review={review}
                  gameId={gameId}
                  onViewFull={() => setSelectedReview(review)}
                />
              ))
            ) : (
              <div className="p-4 bg-surface-dark rounded-lg text-center">
